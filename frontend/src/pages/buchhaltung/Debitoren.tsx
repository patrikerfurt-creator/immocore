...
import type { PersonenkontoSaldo, KontoauszugPosition, SEPAMandat } from '../../types'
...
interface DebFilters { kontonummer: string; eigentuemer_name: string; einheit_nr: string; saldo: string; status: string; sepa: string }
const DEB_EMPTY: DebFilters = { kontonummer: '', eigentuemer_name: '', einheit_nr: '', saldo: '', status: '', sepa: '' }
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
                ...
                {sorted.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="text-center py-8 text-gray-400">
                      {hasFilters ? 'Keine Einträge entsprechen den Filterkriterien.' : 'Keine Personenkonten vorhanden'}
                    </td>
                  </tr>
                ) : (
                  sorted.map(k => (
          ...
          <div className="flex items-center justify-between px-1">
            ...
            {hasFilters && (
              <button type="button" onClick={() => setFilters(DEB_EMPTY)} className="text-xs text-primary-600 hover:text-primary-700 underline">
                Filter zurücksetzen
              </button>
            )}
          </div>
        ...
        <div className="bg-white rounded-lg border overflow-hidden">
          ...
          {(data?.positionen ?? []).some(p => p.hat_detail) && (
            <div className="px-4 py-2 text-xs text-gray-400 border-t bg-gray-50">
              Buchungen mit › anklicken für Abrechnungsarten-Detail
            </div>
          )}
        </div>
      )}
...
// ---------------------------------------------------------------------------
// Ebene 3: Buchungs-Detail (Teilbuchungen / Abrechnungsarten)
// ---------------------------------------------------------------------------
