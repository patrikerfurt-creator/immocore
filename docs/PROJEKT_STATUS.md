...
| `/portal/vorgang-typen/` | im Portal auslösbare Typen (`portal_erstellbar=True, aktiv=True`) | ✅ |
| `/portal/vorgaenge/` (+ `<id>/`) | Liste + Detail mit Verlauf (nur `intern=False`) + Anlage (`quelle='portal'`, Throttle `portal_vorgang_erstellen` 20/Tag) | ✅ |
| `/portal/personenkonto/saldo/` | Saldo je Personenkonto des Eigentümers, aus derselben Rechnung wie intern `mit-saldo` | ✅ |
| `/portal/faelligkeiten/` | offene/teilbezahlte Sollstellungen der eigenen EVs, paginiert (50) | ✅ |
...
| | MeineEinheiten | ✅ | WEG-Buttons → Einheiten-Buttons → Reiter Konto / Dokumente / Vorgänge |
| | KontoReiter | ✅ | Saldo-Karte (Guthaben/Rückstand) mit aufklappbarer Aufschlüsselung + „Nächste Fälligkeiten"; Dokumente-Reiter bleibt Platzhalter |
| | VorgaengeReiter | ✅ | Vorgangsliste → Detail mit Verlauf → Formular „Neuer Vorgang"; Status-Labels portalseitig gemappt |
...
| Abw. 011 | Portal-Erweiterung Vorgänge/Saldo/Fälligkeiten (`CLAUDE_CODE_ANLEITUNG_PORTAL_VORGAENGE_SALDO_FAELLIGKEITEN_v1_1.md`): Die von Kap. 6.1 verlangte Saldo-Aufschlüsselung **je Unterkonto** ist im Datenmodell nicht definierbar — die Soll-Seite hängt am `EigentumsVerhaeltnis` (bzw. je `SollstellungSplit` an einer Buchungsart), die Haben-Seite am `Personenkonto`; eine Zuordnung Buchungsart→Unterkonto existiert nicht. Jede Verteilung wäre eine zweite Saldo-Berechnung, die Kap. 6.2 ausdrücklich verbietet. Stattdessen wird nach `sollstellungs_typ` aufgeschlüsselt plus eine Zeile „Ihre Zahlungen" — dieselben Zahlen, die den Gesamtsaldo bilden (Testzusicherung: Summe der Aufschlüsselung == Gesamtsaldo). Zweite Abweichung: der objekt-Zweig der Sichtbarkeitsregel (Kap. 4.2) gilt nur für einheitenlose Vorgänge, sonst würde ein WEG-Treffer den Vorgang zur Nachbareinheit offenlegen. Dritte: `Vorgang.erstellt_von` ist ein Pflicht-FK auf `auth.User`, Portal-Identitäten haben bewusst keinen — dafür der technische System-User `immocore-portal` (Muster: `immocore-autopilot`); Antragsteller bleibt über `Vorgang.person` + `quelle='portal'` nachvollziehbar | Gering | Umgesetzt 2026-09-09, lokal |
